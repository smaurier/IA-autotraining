...
function parseClaudeMd(content: string): { sections: Section[] } {
  ...
  for (let i = 0; i < matches.length; i++) {
    const end = i + 1 < matches.length
      ...
      : content.length;
    const rawContent = content.slice(matches[i].start, end).trim();
    sections.push({ title: matches[i].title, content: rawContent });
  }

  return { sections };
}

function generateClaudeMd(config: ClaudeMdConfig): string {
  const lines: string[] = [`# ${config.project}`, ''];
  ...
  for (const cmd of config.commands) {
    lines.push(`- **${cmd.name}**: \`${cmd.cmd}\``);
  }

  return lines.join('\n');
}

function buildCursorRules(rules: string[]): string {
  return rules.join('\n');
}

function analyzeCodeReviewPrompt(diff: string, focus: string): string {
  return [
    'Analyse le diff suivant et concentre-toi sur : ' + focus,
    '',
    '```diff',
    diff,
    '```',
...
function estimateProductivityGain(
  ...
  avgTimeWithout: number,
): ProductivityGain {
  const speedup = avgTimeWithout / avgTimeWithAi;
  const timeSaved = (tasksWithout * avgTimeWithout) - (tasksWithAi * avgTimeWithAi);
  return { speedup, timeSaved };
}
