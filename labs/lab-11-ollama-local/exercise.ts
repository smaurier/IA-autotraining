...
function parseStreamResponse(_chunks: string[]): string {
  throw new Error('TODO: Not implemented');
}
...
function buildModelfile(
  _from: string,
  _system: string,
  _params: { name: string; value: string | number }[],
): string {
  throw new Error('TODO: Not implemented');
}
...
function estimateVramGB(_paramsBillion: number, _quantBits: number): number {
  throw new Error('TODO: Not implemented');
}
...
function selectBestModel(
  _available: { name: string; params: number; quality: number }[],
  _maxVram: number,
